...
select employees.last_name,jobs.job_title,employees.manager_id
 ...
 from hr.employees
 where job_id in("SA_R" , "ST_CLERK")
 and salary not in (2500,3500,5000);
 
 
 
 -- 13 
 select max(salary),min(salary),avg(salary),commission_pct
 from employees
 where commission_pct > 0
...
-- 3 function
-- 1
select last_name,concat(upper( substring(last_name,2,1)),lower(substring(last_name,3,3))) as '2-5 last name'
 from employees;
...
case
when salary/2 >10000 then round(salary+ salary*0.10 +1500,2)
when salary/2 <10000 then round(salary+salary*0.115 +1500,2)
end as bonus_for_employee
from employees;
...
-- 7
 ...
 SELECT e1.last_name, e2.department_id, e2.last_name AS coworker_name
FROM employees e1
JOIN employees e2
...
from employees join jobs on employees.job_id=jobs.job_id
where jobs.job_title= employees.department_id = 80;
